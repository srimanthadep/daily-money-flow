...
function SortableRow({
  ...
  return (
    ...
    >
      {/* Drag handle */}
      <td className="px-2 py-4 w-10 text-center border-l border-transparent no-print">
        {!readOnly ? (
          <button
            {...attributes}
            {...listeners}
      ...
      {/* Status */}
      <td className="px-4 py-4 text-center hidden sm:table-cell w-[110px] no-print">
        <span
      ...
      {/* Actions */}
      {!readOnly && (
        <td className="px-3 py-4 text-right w-[100px] no-print">
...
export function EntryTable({
  ...
  return (
    <div className="animate-fade-in relative">
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-slate-50 bg-slate-50/30">
                    <th className="px-2 py-4 w-10 no-print" />
                    ...
                    <th className="px-4 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest text-center hidden sm:table-cell w-[110px] no-print">
                      Status
                    </th>
                     {!readOnly && (
                      <th className="px-3 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest text-right w-[100px] no-print">
                        Actions
                      </th>
                    )}
                  </tr>
                ...
                <tfoot className="no-print">
                  <tr className="bg-slate-50/20 border-t border-slate-100">
                    ...
                    <td className="hidden sm:table-cell no-print" />
      ...
      {/* Print-only Total Row (Guaranteed at the end) */}
      <div className="print-only mt-0 border-t-2 border-slate-200 pt-4">
        <div className="flex justify-between items-center px-4 py-2 bg-slate-50/50 rounded-lg">
          <div className="flex flex-col">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Total Members</span>
            <span className="font-black text-slate-900 text-lg">{entries.length} members</span>
          </div>
          <div className="text-right flex flex-col">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Final Total</span>
            <span className="font-black text-indigo-600 text-2xl tracking-tight">{fmt(total)}</span>
            {pendingTotal !== total && (
        ...
        <div className="mt-8 text-center border-t border-slate-100 pt-4">
          <p className="text-[10px] text-slate-400 font-medium">End of Money Flow Report • Generated on {new Date().toLocaleString()}</p>
        </div>
      </div>
    </div>
  );
